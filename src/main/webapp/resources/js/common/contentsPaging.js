...
paging.initLayout = function() {
	...
	var	warp 		= doc.createElement('ul');
	var	firstLi		= doc.createElement('li');
	var	endLi		= doc.createElement('li');
	var	prevtLi		= doc.createElement('li');
	var	nextLi		= doc.createElement('li');
	var	numArr 		= [];
	var	tempNo 		= 0;
	var	ix 			= 0;
	...
	first.innerHTML = '<img src="/Images/common/prev2.gif"  alt="그 이전" />';
	prev.innerHTML  = '<img src="/Images/common/prev.gif"  alt="이전" />';
	next.innerHTML 	= '<img src="/Images/common/next.gif"  alt="다음" />';
	end.innerHTML  	= '<img src="/Images/common/next2.gif"  alt="그 다음" />';
	
	firstLi.appendChild(first)
	prevtLi.appendChild(prev)
	
	warp.appendChild(firstLi);
	warp.appendChild(prevtLi);
	...
	for(ix = this.startPageNo ; ix < (this.endPageNo +1); ix++){
		if(this.finalPageNo >= ix){
			...
			tempLi.appendChild(tempEl);
			warp.appendChild(tempLi);
		}
	}
...
paging.setOnClass = function () {
	...
	for(; ix < ixLen; ix++){
		...
		if(getPageNoList[ix].textContent == this.pageNo){
			...
			getPageNoList[ix].parentElement.className = 'on';
